...
class Solution(object):
    def solveSudoku(self, board):
        row_candidates =
        {i: set([str(n) for n in range(1, 10)]) for i in range(9)}
        col_candidates =
        {i: set([str(n) for n in range(1, 10)]) for i in range(9)}
        square_candidates =
        {i: set([str(n) for n in range(1, 10)]) for i in range(9)}
        ...
        def recurse(idx, curr):
            ...
            for val in candidates:
                if
                val in row_candidates[i]
                and val in col_candidates[j]
                and val in square_candidates[s_idx]:
                    curr.append([val, i, j])
